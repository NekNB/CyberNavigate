...
SECRET_PATH="/run/secrets/mongo_secret"
# SECRET_PATH="/secrets/mongo.secret.env"

transform_template_to_js() {
	# Обработка SQL шаблонов
	for template in /docker-entrypoint-initdb.d/*.template.js; do
		echo "Work with $template"
		if [ -f "$template" ]; then
				filename=$(basename "$template" .template.js).js
