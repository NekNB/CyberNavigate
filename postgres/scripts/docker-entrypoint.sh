...
# Функция для получения credentials из Docker Secrets
load_secrets() {
...
# Загружает различные настройки, используемые в других частях скрипта
# Эту функцию следует вызывать перед любыми другими функциями
docker_setup_env() {
	load_secrets
...
_main() {
	# Выдаем postgres пользователю права на изменение docker-entrypoint-initdb.d
	chown -R postgres:postgres /docker-entrypoint-initdb.d/
	

	# Если первый аргумент выглядит как флаг, предположим, что мы хотим запустить сервер PostgreSQL.
	if [ "${1:0:1}" = '-' ]; then
		set -- postgres "$@"
	fi
